...
export default function AdminExchangePage() {
  // orders 조인 결과를 포함하는 확장 타입
  type ExchangeWithOrder = ExchangeRefundRequest & {
    orders: { order_number: string; total_amount: number } | null;
  };

  const [requests, setRequests] = useState<ExchangeWithOrder[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [filterStatus, setFilterStatus] = useState<ExchangeStatus | 'all'>('all');
  const [selected, setSelected] = useState<ExchangeWithOrder | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    const { data } = await q;
    setRequests((data ?? []) as ExchangeWithOrder[]);
    setLoading(false);
  }, [filterStatus]);
  ...
  const updateStatus = async (id: string, status: ExchangeStatus) => {
    setUpdatingId(id);
    const payload: { status: ExchangeStatus; admin_memo?: string } = { status };
    if (adminMemo.trim()) payload.admin_memo = adminMemo;
    ...
    if (error) toast.error('변경 실패');
      ...
      load();
      setSelected(s => s ? { ...s, status, admin_memo: adminMemo || s.admin_memo } : null);
    }
    setUpdatingId(null);
  };
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map(r => (
                <tr key={r.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-xs text-gray-500">{r.orders?.order_number ?? '-'}</td>
