  ...
  const searchOrder = async () => {
    ...
    try {
      ...
      if (orderNumber.trim()) {
        ...
        const { data: customerRaw } = await supabase.from('customers').select('id').eq('name', name).eq('phone', phone).maybeSingle();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (existing) {
        ...
        const customerPayload: { name: string; phone: string } = { name, phone };
        const { data: newCRaw, error } = await supabase
          .from('customers')
          .insert(customerPayload as never)
          .select('id')
          .single();
        const newC = newCRaw as { id: string } | null;
      ...
      const { error } = await supabase
        ...
        .insert(exchangePayload as never);
