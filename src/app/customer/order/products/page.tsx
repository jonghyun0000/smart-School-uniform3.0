...
function FallbackProductList({
  ...
  const rawItems = tab === '동복' ? [
    winterData?.top ? { name: '상의(동복)', price: winterData.top, category: '교복상의', uniform_type: '동복' } : null,
    winterData?.bottom ? { name: '하의(동복)', price: winterData.bottom, category: '교복하의', uniform_type: '동복' } : null,
    winterData?.vest ? { name: '조끼/스웨터', price: winterData.vest, category: '조끼', uniform_type: '동복' } : null,
    winterData?.shirt ? { name: '셔츠/T', price: winterData.shirt, category: '셔츠', uniform_type: '동복' } : null,
    winterData?.tie ? { name: '넥타이/리본', price: winterData.tie, category: '넥타이', uniform_type: '동복' } : null,
  ] : [
    summerData?.top ? { name: '상의(하복)', price: summerData.top, category: '교복상의', uniform_type: '하복' } : null,
    summerData?.bottom ? { name: '하의(하복)', price: summerData.bottom, category: '교복하의', uniform_type: '하복' } : null,
    summerData?.tie ? { name: '타이/리본', price: summerData.tie, category: '넥타이', uniform_type: '하복' } : null,
  ];
  const items: FallbackItem[] = rawItems.filter((x): x is FallbackItem => x !== null);
        ...
        <div key={i} className="card p-4 flex items-center gap-4">
          ...
          <Button
            ...
            onClick={() => onAdd({
              ...
              uniform_type: item.uniform_type as Product['uniform_type'],
              gender: '공용',
