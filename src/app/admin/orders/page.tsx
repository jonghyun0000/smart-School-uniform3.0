...
export default function AdminOrdersPage() {
  ...
  const updateStatus = async (orderId: string, newStatus: OrderStatus) => {
    ...
    const payload: { status: OrderStatus } = { status: newStatus };
    ...
    try {
      ...
      for (const item of order.order_items ?? []) {
        ...
        const { data: invRaw } = await supabase
          ...
          .single();
        const inv = invRaw as { quantity: number } | null;
        if (!inv || inv.quantity < item.quantity) {
          toast.error(`${item.product_name} 재고가 부족해요`);
          setUpdatingId(null);
          return;
        }
        const invPayload: { quantity: number } = { quantity: inv.quantity - item.quantity };
        await supabase
          .from('inventory')
          .update(invPayload as never)
          .eq('variant_id', item.variant_id);
        ...
        await supabase.from('sales_logs').insert(logPayload as never);
      }
      ...
      await supabase
        .from('orders')
        .update(confirmPayload as never)
        .eq('id', order.id);
