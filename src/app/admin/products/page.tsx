...
import { formatPrice, SIZE_OPTIONS } from '@/lib/utils';
import type { Product, School } from '@/types/database';
import toast from 'react-hot-toast';
import { Plus, RefreshCw, Search, Edit2, Save, X, ChevronDown, ChevronUp } from 'lucide-react';

export default function AdminProductsPage() {
  // schools 조인 결과를 포함하는 확장 타입
  type ProductWithSchool = Product & {
    schools: { name: string } | null;
  };

  const [products, setProducts] = useState<ProductWithSchool[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setProducts((prodRes.data ?? []) as ProductWithSchool[]);
  ...
  const saveProductPrice = async (productId: string, base_price: number, name: string) => {
    setSaving(true);
    const payload: { base_price: number; name: string } = { base_price, name };
    const { error } = await supabase
      .from('products')
      .update(payload as never)
      .eq('id', productId);
  ...
  const toggleActive = async (productId: string, current: boolean) => {
    const payload: { is_active: boolean } = { is_active: !current };
    const { error } = await supabase
      .from('products')
      .update(payload as never)
      .eq('id', productId);
  ...
  const addVariant = async (productId: string, size: string, price: number) => {
    const variantPayload: { product_id: string; size: string; price: number; is_active: boolean } = {
    ...
    if (error) toast.error('추가 실패: ' + error.message);
      ...
      const { data: vRaw } = await supabase
        ...
        .eq('size', size)
        .single();
      const v = vRaw as { id: string } | null;
      if (v) {
        const invPayload: { variant_id: string; quantity: number; reserved_quantity: number } = {
          variant_id: v.id,
          quantity: 0,
          reserved_quantity: 0,
        };
        await supabase.from('inventory').insert(invPayload as never);
      }
      load();
    }
  };
  ...
  const filtered = products.filter(p => {
    ...
    return p.name.toLowerCase().includes(s) || (p.schools?.name ?? '').toLowerCase().includes(s) || p.category.includes(s);
  });
                ...
                <div className="flex-1">
                  {editingProduct?.id === product.id ? (
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-gray-400">{product.schools?.name ?? '공통'}</span>
