  ...
  const handleOrder = async () => {
    ...
    try {
      ...
      const { data: existingCustomerRaw } = await supabase
        ...
        .maybeSingle();
      const existingCustomer = existingCustomerRaw as { id: string } | null;
      ...
      if (existingCustomer) {
        ...
        const { data: newCustomerRaw, error: custError } = await supabase
          .from('customers')
          .insert(customerInsertPayload as never)
          .select('id')
          .single();
        const newCustomer = newCustomerRaw as { id: string } | null;
      ...
      const { data: orderRaw, error: orderError } = await supabase
        .from('orders')
        .insert(orderInsertPayload as never)
        .select('id, order_number')
        .single();
      const order = orderRaw as { id: string; order_number: string } | null;
      ...
      const items: OrderItemInsert[] = cart.map(item => ({
        order_id: order.id,
        product_id: item.product_id,
        variant_id:
          item.variant_id.startsWith('fallback') ||
          item.variant_id.startsWith('pe') ||
          item.variant_id.startsWith('nameplate')
            ? null
            : item.variant_id,
